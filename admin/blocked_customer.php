...
<section class="content-header" style="display:flex;justify-content:space-between">
		...
		<h1 class="col-lg-3">Active Customers</h1>
	
		<a href="customers.php" class="btn btn-primary btn-sm" style="float:right;margin:0px 750px;">Active Customers</a>
</section>
				...
				<div class="">
					...
					<thead class="thead-dark">
						...
						<tbody>
							<?php
							$i=1;
							$result = $db->prepare("select * from customer where status='0'");
								$result->execute();
								for($i=0; $row = $result->fetch(); $i++)
								{
									...
									<tr>
										<td><?php echo $i; ?></td>
										<td style=""><?php echo $row['f_name']." ".$row['l_name']; ?></td>
										<td class="col-lg-2"><?php echo $row['email']; ?></td>
										<td><?php echo $row['phone']; ?></td>
										<td><?php echo $row['city']; ?></td>
                                        <td style="color:green;">Blocked</td>
										<td><a href="c_unblock.php?id=<?php echo $row['id']; ?>" class="btn btn-success">UnBlock</a></td>
									</tr>
									<?php
								}
