...
<section class="content-header" style="display:flex;justify-content:space-between">
		...
		<h1 class="col-lg-3">Pending Orders</h1>
			...
			<div class="box box-info">
				<div class="">
					...
					<thead class="thead-dark">
						...
						<tbody>
							<?php
							$result = $db->prepare("select p.name as p_name,p.id as p_id,p.customizable as customizable,pay.status as p_status,o.c_id as c_id,op.* from product p,payment pay,orders o,ordered_products op where op.order_id=o.id and o.id=pay.order_id and op.p_id=p.id and p.s_id='$seller_id' and op.status IN(1,2) order by date DESC");
								$result->execute();
								for($i=1; $row = $result->fetch(); $i++)
								{
									...
									<tr>
										...
										<td class="col-lg-2">										
											<?php if($row['p_status']==1)echo "<div style='color:green;'>SUCCESS</div>";else echo "<div style='color:red;'>PENDING</div>";
                                             ?>  
										</td>
                                        <td> 
                                            <?php
                                            if($row['status']==1){?>										
											 <a href="dispatch.php?id=<?php echo $row['id']; ?>" class="btn btn-success">Dispatched</a> 
                                             <?php
                                            }
                                            else{?>
                                            <a href="delivered.php?id=<?php echo $row['id']; ?>" class="btn btn-success">Delivered</a> 
                                            <?php } ?>
